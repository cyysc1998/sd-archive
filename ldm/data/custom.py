...
class Txt2ImgIterableBaseDataset(IterableDataset):
    ...
    '''
    def __init__(self, file_path: str, txt_path: str, img_size, rank, world_size):
        super().__init__()
        ...
        self.info = self._get_file_info(file_path, txt_path)
    ...
    def _get_file_info(self, file_path, txt_path):
        info = \
        {
            "start": 1,
            "end": 0,
        ...
        for img in self.img_list:
            self.file_list.append(os.path.join(file_path, img))
            self.txt_list.append(os.path.join(txt_path, img.split('.')[0] + '.txt'))
        ...
        #     txts = [k.replace('png', 'txt') for k in files]
...
if __name__ == '__main__':
    custom_data = Txt2ImgIterableBaseDataset(
        file_path='/data2/sun/data/FFHQ-Text-Dataset/Image/00_Female/',
        txt_path='/data2/sun/data/FFHQ-Text-Dataset/Text/00_Female/', 
        rank=0, 
        world_size=1)
    custom_dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
        worker_init_fn=lambda x: np.random.seed(x + int(torch.initial_seed()) % 2 ** 32),
    )
